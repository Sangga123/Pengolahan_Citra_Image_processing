...
"")
    ...
    def convertToGreyscaleAverage(self):
        ...
        #     # average = self.pixmap1.toImage()  # Konversi QPixmap ke QImag
        #     image = self.image.convertToFormat(QtGui.QImage.Format_RGB888) # mengubah object image menjadi format RGB888

        #     width = image.width() # iniasiasi dan mengambil lebar gambar
        #     height = image.height() # iniasiasi dan mengambil tinggi gambar

        #     for y in range(height): # looping berdasarkan tinggi gambar
        #         for x in range(width): # looping berdasarkan lebar gambar
        #             pixel = image.pixel(x, y) # iniasiasi object pixel
        #             r = QtGui.qRed(pixel)
        #             g = QtGui.qGreen(pixel)
        #             b = QtGui.qBlue(pixel)
        #             gray = (r + g + b) // 3
        #             image.setPixel(x, y, QtGui.qRgb(gray, gray, gray)) # mengubah nilai pixel gambar menjadi abu-abu

        #     pixmap = QtGui.QPixmap.fromImage(image) # mengubah nilai pada atribut gambar, dari gambar lama ke gambar yang baru
        #     self.pbOutput.setPixmap(pixmap) # menampilkan gambar yang baru ke dalam label output
        #     self.pbOutput.setScaledContents(True) # fungsi auto fit
    ...
    def load_data(self):
        ...
        for i in range(101):
            QtCore.QCoreApplication.processEvents(Ui_MainWindow)  # Allow the GUI to update
            self.progressBar.setValue(i)  # Update the progress bar
            # Simulate some time-consuming operation here, replace this with your actual loading code
            QtCore.QThread.msleep(20)  # Sleep for 20 milliseconds
    ...
    def rotate(self):
        ...
        if ok:
            if hasattr(self, 'pixmap1') and not self.pixmap1.isNull():
                ...
                # Handle the case where self.pixmap1 is not a valid QPixmap
